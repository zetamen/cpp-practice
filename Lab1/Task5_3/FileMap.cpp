...
shared_ptr<Map> FileMap::Read(ifstream& input)
{
...
void FileMap::Write(ofstream & output, shared_ptr<Map> const& map)
{
	Map* mapPtr = map.get();
...
PointValue FileMap::GetPoint(char const ch) const
{
	PointValue point;
	switch (ch)
	{
...
char FileMap::GetChar(PointValue const point) const
{
	char ch;
	switch (point)
	{
